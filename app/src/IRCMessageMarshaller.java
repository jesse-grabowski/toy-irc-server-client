...
public class IRCMessageMarshaller {
  ...
  private String marshalCapLIST(IRCMessageCAPLIST message) {
    ...
    if (message.isHasMore()) {
      return "%s LIST * :%s"
          .formatted(message.getNick(), String.join(" ", message.getCapabilities()));
    } else {
      return "%s LIST :%s"
          .formatted(message.getNick(), String.join(" ", message.getCapabilities()));
    }
  ...
  private String marshalPart(IRCMessagePART message) {
    return String.join(",", message.getChannels())
        + (message.getReason() != null ? " :" + message.getReason() : "");
  }
  ...
  private <T> String marshalMap(
      SequencedMap<String, String> map, String delimiter, Function<String, String> mapper) {
